...
class Individuo(opcoes):
    ...
    def Mutacao(self,indice,tamanho_populacao,populacao,F):
        ...
        vetor_mutante=copy(v0 + F*(v2 - v1))
        return vetor_mutante

    def Cruzamento(self, v_i, limites, Cr):
...
class Populacao_Iteracao:
    def __new__(cls,populacao,tamanho_pop):
        cls.pop=array([])
        dim=len(populacao[0].posicao)
        
        pop_aux=copy(populacao)
        #pop_aux=sort(pop_aux,order='fitness')

        for i in range(tamanho_pop):
            #print("Individuo_i",populacao[i].posicao)
            cls.pop=append(cls.pop,populacao[i])#ACHO QUE TERIA QUE RANQUEAR OS INDIVIDUOS PARA NAO PERDER OS MELHORES

        return cls.pop


class Evolucao_Diferencial_Controle_Pop_Caotico(Individuo):
    Defaut={"F":0.8,
            "CR":0.5,
            "Gmax":100,
            "Tolerancia":10**-5,
    ...
    def __init__(self,FuncaoObjetivo, Tamanho_Populacao_Min,Tamanho_Populacao_Max, limites, OpcoesDE=None):
        ...
        F_calc = append(F_calc, F)
        Cr_calc = append(Cr_calc, Cr)
        ...
        while g<self.Usuario["Gmax"] and fitness_best>=self.Usuario["Tolerancia"]:
            ...
            for i in range(Tamanho_Populacao):
                vetor_mutante    = copy(Populacao[i].Mutacao(i,Tamanho_Populacao,Populacao_Old,F))
                vetor_resultante = copy(Populacao[i].Cruzamento(vetor_mutante,limites,Cr))
            ...
            Populacao_Old=Populacao_Iteracao(Populacao,Tamanho_Populacao)
        ...
        self.Valor_F=copy(F_calc)
        self.Valor_Cr=copy(Cr_calc)
